...
function extractIKMData(rows) {
  ...
  return {title,month,totalRespondents,finalScore,finalScoreText,aspects,
...
function AspectBarChart({aspects}){
  ...
  useEffect(()=>{
    ...
    chart.current=new Chart(ref.current.getContext('2d'),{
      type:'bar',
      data:{labels:aspects.map(a=>a.name),datasets:[
    ...
    chart.current=new Chart(ref.current.getContext('2d'),{
      ...
      options:{responsive:true,maintainAspectRatio:false,
        plugins:{
          ...
          tooltip:{backgroundColor:'#1c1b19',bodyFont:{size:11},padding:10,cornerRadius:8,
        ...
        {loading?(
          ...
          <>
            ...
            <section className="section-card conclusion-section">
              ...
              <p>Meskipun sebagian besar unsur pelayanan sudah berjalan optimal dan masuk dalam kategori baik, RSU Islam Klaten akan terus melakukan langkah-langkah strategis untuk meningkatkan efisiensi waktu layanan demi kenyamanan dan kepuasan pasien yang lebih baik.</p>
            </section>
          </>
        )}
      </main>
