...
function extractIKMData(rows) {
  ...
  for (let i = 23; i <= 30 && i < rows.length; i++) {
...
function PieChart({ labels, values, colors }) {
  ...
  if (!values?.length) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '160px', color: '#bab9b4', fontSize: '0.85rem', gap: '8px' }}>
      ...
      <span>Data tidak tersedia atau sedang terjadi gangguan server</span>
    </div>
  );
      ...
      {sheets.length === 0 ? (
        <div className="sheet-discovering">
          {discovering
            ...
            : <span style={{ color: 'var(--color-error)', fontSize: '0.8rem' }}>Gagal memuat data</span>
          }
        </div>
      ) : (
  ...
  return (
    <div className="app-root">
      ...
      <footer className="app-footer">
        ...
        <p>Dikembangkan oleh <a class="footer-link" href="https://kfmd.notion.site" target="_blank">dr. Khariz Fahrurrozi</a></p>
